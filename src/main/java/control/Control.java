...
//        processRequest(request, response);
        ...
        try {
            um.register(new User(username, password));
        } catch (SQLException ex) {
            ex.printStackTrace();
        }
        //request.getRequestDispatcher("showusers.jsp").forward(request, response); //Using jsp instead of servlet for View.
        response.sendRedirect("Control"); //Done here to create a get-request to this servlet.
    }
